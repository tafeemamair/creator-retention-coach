...
export default function RetentionForm() {
  ...
  async function onAnalyzePreview() {
    ...
    try {
      ...
      if (data.message) setUpgradeMessage(data.message);
    } catch (e) {
  ...
  async function loadRazorpayCheckout(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      scriptTag.onerror = () => reject(new Error("Unable to load Razorpay checkout."));
  ...
  async function handleUnlock() {
    ...
    try {
      ...
      if (!orderResponse.ok || !order.id || !order.amount || !order.currency || !order.keyId) {
      ...
      const checkout = new window.Razorpay({
        ...
        description: "Full retention analysis",
        order_id: order.id,
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <div className="mb-3">
          <label htmlFor="platform" className="mb-1 block text-xs text-slate-300">Platform</label>
          <select
            id="platform"
            value={platform}
      ...
      {preview ? (
        <section className="rounded-xl border border-blue-700/60 bg-slate-900/50 p-5">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="rounded-lg border border-slate-700 bg-slate-950/60 p-3"><p className="text-xs text-slate-400">Hook Strength Score</p><p className="text-2xl font-bold text-blue-300">{Math.round(preview.metrics.hook)} / 100</p></div>
            <div className="rounded-lg border border-slate-700 bg-slate-950/60 p-3"><p className="text-xs text-slate-400">Predicted Drop-off</p><p className="text-2xl font-bold text-amber-300">{preview.dropoffPrediction.second}s</p></div>
            <div className="rounded-lg border border-slate-700 bg-slate-950/60 p-3"><p className="text-xs text-slate-400">Overall Preview Score</p><p className="text-2xl font-bold text-emerald-300">{Math.round(preview.score)} / 100</p></div>
          </div>
          <div className="mt-4 rounded-lg border border-slate-700 bg-slate-950/60 p-4 text-sm text-slate-200">
            ...
            <ul className="mt-2 list-disc space-y-1 pl-5">{retentionInsights.map((insight) => <li key={insight}>{insight}</li>)}</ul>
          </div>
        </section>
      ) : null}
      ...
      {preview && status === "locked" ? (
        <section className="rounded-xl border border-amber-600/60 bg-amber-500/10 p-5">
          ...
          <button onClick={handleUnlock} className="mt-3 rounded-lg bg-amber-500 px-4 py-2 font-medium text-slate-900 hover:bg-amber-400">Unlock Full Analysis — ₹49</button>
        </section>
      ) : null}

      {preview && status === "loading" ? (
        <section className="rounded-xl border border-slate-700 bg-slate-900/50 p-8 text-center text-slate-200">Waiting for payment verification...</section>
      ) : null}
        ...
        <section className="rounded-xl">
          <div className="space-y-6">
            ...
            <section className="rounded-xl border bg-white p-4 text-slate-900 shadow-sm">
              ...
              <ul className="list-disc pl-5 text-sm">{(fullAnalysis?.viralTitleSuggestions ?? ["Loading title suggestions..."]).map((title) => <li key={title}>{title}</li>)}</ul>
            </section>
            {(fullAnalysis?.rewrites ?? [{ type: "Curiosity Hook", script: "Loading improved script versions..." }]).map((rewrite) => <ScriptRewrite key={rewrite.type} original={script} improved={rewrite.script} type={rewrite.type} />)}
          </div>
        </section>
      ) : null}
    </div>
